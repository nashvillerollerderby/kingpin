...
public class TeamJamImpl extends ParentOrderedScoreBoardEventProviderImpl<TeamJam> implements TeamJam {
    ...
    protected Object computeValue(Value<?> prop, Object value, Object last, Source source, Flag flag) {
        ...
        if (prop == NO_INITIAL) {
            if (getCurrentScoringTrip() == null) {
                return true;
            }
            return getCurrentScoringTrip().getNumber() == 1;
        }
        if ((prop == LEAD || prop == LOST) && getJam().isImmediateScoring()) {
            return false;
        }
        ...
        if (value instanceof Integer && prop != OS_OFFSET && (Integer) value < 0) {
            return 0;
        }
        return value;
    }
    ...
    protected void valueChanged(Value<?> prop, Object value, Object last, Source source, Flag flag) {
        if (prop == STAR_PASS_TRIP) {
            if (last != null) {
                getLast(SCORING_TRIP).set(ScoringTrip.AFTER_S_P, false, Flag.SPECIAL_CASE);
            }
            if (value != null) {
    ...
    public void execute(Command prop, Source source) {
        if (prop == COPY_LINEUP_TO_CURRENT) {
            copyLineupToCurrentJam();
        }
    }
    ...
    private void copySkaterToIfAvailable(FloorPosition fp, TeamJam target) {
        ...
        if (placeAvailable) {
            team.field(s, fp.getRole(), target);
        }
    }
    ...
    private final RecalculateScoreBoardListener<?> jamScoreListener;
    private final RecalculateScoreBoardListener<?> afterSPScoreListener;
}
