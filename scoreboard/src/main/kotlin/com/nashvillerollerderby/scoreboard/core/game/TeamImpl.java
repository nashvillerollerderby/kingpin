...
public class TeamImpl extends ScoreBoardEventProviderImpl<Team> implements Team {
    ...
    public TeamImpl(Game g, Team source) {
        ...
        for (ValWithId an : source.getAll(ALTERNATE_NAME)) {
            add(ALTERNATE_NAME, an);
        }
        for (ValWithId c : source.getAll(COLOR)) {
            add(COLOR, c);
        }
        for (Skater s : source.getAll(SKATER)) {
            add(SKATER, new SkaterImpl(this, s));
        }
    }
    ...
    protected Object computeValue(Value<?> prop, Object value, Object last, Source source, Flag flag) {
        ...
        if (prop == UNIFORM_COLOR && "".equals(value)) {
            return null;
        }
        if (prop == IN_TIMEOUT) {
        ...
        if (prop == TRIP_SCORE && source != Source.COPY) {
            ...
            if (game.isInJam() && ((Integer) value > 0 || ((Integer) last == 0 && flag != Flag.CHANGE))) {
                ...
                if (tripScoreJamTime == 0L) {
                    tripScoreJamTime = game.getClock(Clock.ID_JAM).getTimeElapsed();
                }
        ...
        if (prop == SCORE) {
            ...
            for (ScoreAdjustment adjustment : getAll(SCORE_ADJUSTMENT)) {
        ...
        if (prop == INJURY && source != Source.COPY && (Boolean) value) {
            set(CALLOFF, false);
        }
        if (prop == CALLOFF && source != Source.COPY && (Boolean) value) {
            set(INJURY, false);
        }
        ...
        if (prop == TOTAL_PENALTIES) {
            ...
            for (Skater s : getAll(SKATER)) {
                count += s.get(Skater.PENALTY_COUNT);
            }
            return count;
        }
        if (prop == ALL_BLOCKERS_SET) {
            for (Position p : getAll(POSITION)) {
                if (p.getFloorPosition() == FloorPosition.JAMMER) {
                    continue;
                }
                if (p.getSkater() == null) {
                    return false;
    ...
    protected void valueChanged(Value<?> prop, Object value, Object last, Source source, Flag flag) {
        if (prop == LEAD && (Boolean) value && game.isInJam()) {
            ...
            if ((Boolean) value && !getOtherTeam().isLead() && isOnInitial()) {
                set(LOST, true);
            }
    ...
    protected void itemAdded(Child<?> prop, ValueWithId item, Source source) {
        ...
        if (prop == SCORE_ADJUSTMENT) {
            scoreListener.addSource(((ScoreAdjustment) item), ScoreAdjustment.AMOUNT);
        }
        if (prop == SKATER) {
            penaltyListener.addSource((Skater) item, Skater.PENALTY_COUNT);
        }
    }
    ...
    protected <T extends ValueWithId> boolean mayRemove(Child<T> prop, T item, Source source) {
        ...
        return prop != SKATER || item == null || ((Skater) item).getAll(Skater.FIELDING).isEmpty();
    }
    ...
    protected void itemRemoved(Child<?> prop, ValueWithId item, Source source) {
        ...
        if (prop == SCORE_ADJUSTMENT && item == get(ACTIVE_SCORE_ADJUSTMENT)) {
            set(ACTIVE_SCORE_ADJUSTMENT, null);
        }
    }
    ...
    public void stopJam() {
        synchronized (coreLock) {
            if (get(ACTIVE_SCORE_ADJUSTMENT) != null) {
                set(ACTIVE_SCORE_ADJUSTMENT, null, Flag.SPECIAL_CASE);
            }
            ...
            for (Skater s : toField.keySet()) {
                ...
                if (bt != null && bt.isCurrent()) {
                    bt.add(BoxTrip.FIELDING, s.getFielding(upcomingTJ));
                }
            }
    ...
    protected void mergeInto(PreparedTeam pt) {
        ...
        if ("".equals(pt.get(TEAM_NAME))) {
            pt.set(TEAM_NAME, get(TEAM_NAME));
        }
        if ("".equals(pt.get(LEAGUE_NAME))) {
            pt.set(LEAGUE_NAME, get(LEAGUE_NAME));
        }
        if ("".equals(pt.get(LOGO))) {
            pt.set(LOGO, get(LOGO));
        }
        if ("".equals(get(TEAM_NAME))) {
            set(TEAM_NAME, pt.get(TEAM_NAME));
        }
        if ("".equals(get(LEAGUE_NAME))) {
            set(LEAGUE_NAME, pt.get(LEAGUE_NAME));
        }
        if ("".equals(get(LOGO))) {
            set(LOGO, pt.get(LOGO));
        }
        ...
        if (!hasColor) {
            pt.add(PreparedTeam.UNIFORM_COLOR, new ValWithId(get(UNIFORM_COLOR), get(UNIFORM_COLOR)));
        }
        ...
        for (ValWithId c : getAll(COLOR)) {
            if (pt.get(COLOR, c.getId()) == null) {
                pt.add(COLOR, new ValWithId(c.getId(), c.getValue()));
            }
        }
        mergeSkaters(pt);
    ...
    private void mergeSkaters(PreparedTeam pt) {
        ...
        for (PreparedSkater ps : preparedSkaters) {
            ...
            if (preparedByNumberAndName.get(number) == null) {
                preparedByNumberAndName.put(number, new HashMap<>());
            }
        ...
        for (Skater s : localSkaters) {
            ...
            try {
                ...
                if (preparedByNumberAndName.get(number).isEmpty()) {
                    preparedByNumberAndName.remove(number);
                }
        ...
        for (Skater s : localSkaters) {
            if (s.get(Skater.PREPARED_SKATER) != null) {
                continue;
            }
            String name = s.get(Skater.NAME);
            try {
                ...
                if (preparedByNumberAndName.get("").isEmpty()) {
                    preparedByNumberAndName.remove("");
                }
            } catch (NullPointerException e) {
            }
        }
        ...
        for (Skater s : localSkaters) {
            ...
            String number = s.get(Skater.ROSTER_NUMBER);
            if ("".equals(number) || !preparedByNumberAndName.containsKey(number)) {
                continue;
            }
    ...
    public void timeout() {
        synchronized (coreLock) {
            if (getTimeouts() > 0) {
                game.setTimeoutType(this, false);
            }
        }
    }
    ...
    public boolean cancelTripAdvancement() {
        return tripScoreTimerTask.cancel();
    }
    ...
    private final Timer tripScoreTimer = new Timer();
    private TimerTask tripScoreTimerTask = new TimerTask() {
        @Override
        public void run() {
        } // dummy, so the variable is not
    ...
    private final RecalculateScoreBoardListener<?> scoreListener;
    private final RecalculateScoreBoardListener<?> penaltyListener;
